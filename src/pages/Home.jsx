...
function Home() {
  ...
  const getUserInfo = async () => {
    try {
      const response = await axiosIntance.get("/users/get-user");
  ...
  const getAllNotes = async () => {
    try {
      const response = await axiosIntance.get("/notes/getNotes");
  ...
  const deleteNote = async (data) => {
    ...
    try {
      const response = await axiosIntance.delete(`/notes/note-delete/${noteID}`); // Ensure /api is added if your routes are prefixed
  ...
  const SearchNote = async (query)=> {
   try {
     const response = await axiosIntance.get("/notes/search-note", {params : {query} })
  ...
  const updateIsPinned = async (note) => {
    ...
    try {
      ...
      const response = await axiosIntance.put(`/notes/isPinned/${noteID}`, {
  ...
  className="h-[80vh] flex items-center justify-center flex-col text-2xl font-semibold gap-2 w-ful"
>
